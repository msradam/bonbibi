...
**The board went unresponsive before the deadline.** The touchscreen for the kiosk demo arrived late, leaving a narrow window to wire it up and record a live on-device capture; the Pi 5 stopped responding on the network shortly after (unresponsive at the OS level, no ping/ARP/SSH, though powered) before that recording could happen. The numbers in this writeup are not invented to cover the gap: they come from `bench_llm_cpu.log`, `spec_decode.log`, and `BENCHMARK_RESULTS.md`, all captured directly on the Pi earlier the same day, before the board went down, and every one of those numbers is traceable to a specific line in those raw files. What's missing is a fresh live-hardware video capture, not the measurements themselves.

Recovered logs from the board's SD card (`sdcard_recovery/`, pulled read-only via `debugfs` after the crash, since SSH could no longer reach it) are corroborating, not conclusive: none of the four files carry a date in their own content, so the same-day attribution rests on when they were pulled, not on a timestamp a reader can independently check. What they do show: `bonbibi_server.log` records Uvicorn serving `GET /panel/ HTTP/1.1" 200 OK`, and `cog.log` records the kiosk browser loading that page successfully, both consistent with the console having been live. The last attempt's `llama_server.log` shows the narrator backend failing to start (`env: 'llama.cpp/build-vulkan/bin/llama-server': No such file or directory`, a stale binary path in the launcher, not a hardware fault), so that run had the map and console live without spoken guidance. What's not recovered: the kernel panic itself, no dmesg or journal output showing the actual crash, just these four application-level logs from the attempt right after it.
