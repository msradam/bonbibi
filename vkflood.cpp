// Flux-limited (WCA2D / virtual-pipes) overland flow on the Pi 5 V3D.
// Two passes per step (flux.spv then height.spv), ping-pong water buffers, a
// bowl terrain + uniform rain. Verifies the GPU result against a double CPU
// reference (NMSE), checks mass conservation vs rain injected, and that water
// pools in the basin. Build: g++ -O3 -o vkflood vkflood.cpp -lvulkan
...
int main(int argc, char** argv){
    ...
    const float rain  = getenv("RAIN") ? atof(getenv("RAIN")) : 0.02f;  // m/step; tune vs terrain relief
    ...
    VkPipeline pFlux=mkPipe(dev,pl,"flux.spv"), pHeight=mkPipe(dev,pl,"height.spv");

    // buffers: 0=H, 1=W0, 2=W1, 3=Flux(vec4)
    VkDeviceSize wbytes=(VkDeviceSize)SZ*SZ*4, fbytes=(VkDeviceSize)SZ*SZ*16;
    VkBuffer buf[4]; VkDeviceMemory mem[4]; void* map[4];
    VkDeviceSize sizes[4]={wbytes,wbytes,wbytes,fbytes};
    for(int i=0;i<4;i++){
    ...
    float* H=(float*)map[0]; float* W0=(float*)map[1]; float* W1=(float*)map[2];
    for(uint32_t i=0;i<SZ*SZ;i++){ W0[i]=0; W1[i]=0; }
    const char* dempath=getenv("DEM");
    float la0=0,la1=0,lo0=0,lo1=0; bool geo=false;
    if(dempath){  // real terrain: nearest-upsample a DEM to SZxSZ, floor to min elevation
    ...
    auto wire=[&](VkDescriptorSet s,int win,int wout){
        VkDescriptorBufferInfo dbi[4]={{buf[0],0,wbytes},{buf[win],0,wbytes},{buf[wout],0,wbytes},{buf[3],0,fbytes}};
        // binding: 0=H,1=Win,2=Flux,3=Wout  -> reorder: dbi index != binding, set explicitly
        VkDescriptorBufferInfo b0{buf[0],0,wbytes}, b1{buf[win],0,wbytes}, b2{buf[3],0,fbytes}, b3{buf[wout],0,wbytes};
        ...
        vkUpdateDescriptorSets(dev,4,w,0,nullptr); (void)dbi;
    };
    wire(ds[0],1,2); wire(ds[1],2,1);
    ...
    uint32_t G=(SZ+15)/16; PC pc{SZ,k,rain};
    ...
    // CPU reference: same 2-pass scheme in double
    ...
    for(int s=0;s<N;s++){
        for(uint32_t y=0;y<SZ;y++) for(uint32_t x=0;x<SZ;x++){uint32_t i=y*SZ+x; double hi=H[i]+a[i];
            double fL=(x>0)?std::max(0.0,k*(hi-(H[i-1]+a[i-1]))):0, fR=(x<SZ-1)?std::max(0.0,k*(hi-(H[i+1]+a[i+1]))):0;
            double fU=(y>0)?std::max(0.0,k*(hi-(H[i-SZ]+a[i-SZ]))):0, fD=(y<SZ-1)?std::max(0.0,k*(hi-(H[i+SZ]+a[i+SZ]))):0;
    ...
    for(uint32_t i=0;i<SZ*SZ;i++){double d=(double)gpu[i]-a[i]; num+=d*d; den+=a[i]*a[i]; tot_gpu+=gpu[i]; tot_cpu+=a[i]; if(gpu[i]>maxd){maxd=gpu[i];argmax=i;}}
    ...
    // Dump a coarse depth grid for the agent to read (perception -> cognition link).
    { FILE* f=fopen("/tmp/flood_depth.txt","w"); const uint32_t D=16, step=SZ/D;
      ...
      for(uint32_t gy=0;gy<D;gy++){ for(uint32_t gx=0;gx<D;gx++){
          uint32_t x=gx*step+step/2, y=gy*step+step/2; fprintf(f,"%5.2f ", gpu[y*SZ+x]); } fprintf(f,"\n"); }
